# HR-Attrition-Analysis-Tableau
Interactive HR Attrition Analysis Dashboard built using Tableau to identify employee turnover patterns, workforce trends, and business insights.
# Data Cleaning Report :
The objective of data cleaning was to improve the quality, consistency, and usability of the HR employee attrition dataset before performing exploratory data analysis and building an interactive Tableau dashboard.

 Dataset Information:
  Attribute	Details
  Dataset	HR Employee Attrition
  Total Records	1,470 Employees
  Total Columns	35
  Tool Used	Microsoft Excel
  Visualization Tool	Tableau
  
# Data Cleaning Steps:
1. Removed Unnecessary Columns:
 The following columns contained constant values and did not contribute to the analysis.
 Column	Reason
EmployeeCount	Constant value (1)
Over18	Constant value (Y)
StandardHours	Constant value (80)
2. Checked Missing Values
No missing values were found.
Dataset was complete.
3. Checked Duplicate Records
Duplicate employee records were verified.
No duplicate records were found.
4. Data Type Validation
Verified numerical and categorical columns.
Examples:
  Numerical:
   Age
   Monthly Income
   Daily Rate
   Years at Company
   Job Satisfaction
   Environment Satisfaction
 Categorical:
   Gender
   Department
   Job Role
   Attrition
   Business Travel
   Education Field
   Marital Status

# EDA Report :
Key Performance Indicators:
Total Employees
Employees Left
Active Employees
Attrition Rate
Average Monthly Income
Average Age
Average Years at Company
Average Job Satisfaction

# Visualizations Developed:
1. Attrition by Gender

Objective

Analyze employee attrition by gender.

Insight

Male employees contribute a higher proportion of employee attrition compared to female employees.

2. Attrition by Age Group

Objective

Identify which age groups experience the highest employee attrition.

Insight

Employees in the younger and mid-career age groups show comparatively higher attrition.

3. Attrition by Department

Objective

Analyze employee attrition across departments.

Insight

Research & Development experiences the highest employee attrition.

4. Attrition by Job Role

Objective

Identify job roles with higher employee turnover.

Insight

Sales Executive and Laboratory Technician record the highest attrition.

5. Monthly Income vs Attrition

Objective

Study the relationship between salary and employee attrition.

Insight

Employees with lower monthly income tend to leave more frequently.

6. Overtime vs Attrition

Objective

Analyze the impact of overtime on attrition.

Insight

Employees working overtime exhibit a significantly higher attrition rate.

7. Job Satisfaction vs Attrition

Objective

Evaluate employee satisfaction.

Insight

Lower job satisfaction is associated with increased employee attrition.

8. Attrition by Years Since Last Promotion

Objective

Study promotion history.

Insight

Employees with longer periods since their last promotion demonstrate higher attrition.

9. Employee Career Progression

Objective

Categorize employees into career stages.

Insight

Early-career employees represent the largest workforce segment.

10. Education Field vs Attrition

Objective

Analyze attrition across education fields.

Insight

Life Sciences and Medical education fields contribute the largest number of employee exits.

11. Department Ranking by Attrition



Research & Development ranks first in employee attrition.

# Key Business Insights:
Workforce Overview
Total workforce consists of 1,470 employees.
Employee attrition rate is approximately 16%.
Active employees account for the majority of the workforce.
Department Analysis

Research & Development department records the highest employee attrition.

Recommendation

Improve employee engagement.
Conduct regular feedback sessions.
Strengthen career development initiatives.
Job Role Analysis

Sales Executive and Laboratory Technician experience comparatively higher attrition.

Recommendation

Review workload.
Enhance career growth opportunities.
Introduce role-specific retention strategies.
Salary Analysis

Lower monthly income is associated with higher employee turnover.

Recommendation

Evaluate compensation policies.
Introduce performance-based incentives.
Review salary competitiveness.
Overtime Analysis

Employees working overtime exhibit increased attrition.

Recommendation

Balance employee workload.
Promote work-life balance.
Monitor overtime allocation.
Promotion Analysis

Employees who have not received promotions for several years are more likely to leave.

Recommendation

Establish transparent promotion policies.
Increase internal career opportunities.
Recognize employee performance regularly.
Job Satisfaction

Lower job satisfaction corresponds with increased employee attrition.
Recommendation
Improve employee engagement programs.
Conduct satisfaction surveys.
Address workplace concerns promptly.
Career Progression

Early-career employees account for a significant portion of the workforce.

Recommendation

Strengthen onboarding.
Provide mentoring programs.
Offer structured career development plans.
Strategic Recommendations
Improve employee retention initiatives.
Strengthen leadership development.
Review compensation and benefits.
Enhance work-life balance policies.
Increase employee recognition programs.
Invest in continuous learning and development.
Conduct periodic employee engagement surveys.
Use HR analytics for proactive decision-making.


# The HR Attrition Analysis Dashboard enables organizations to monitor workforce trends, identify critical attrition factors, and support evidence-based HR decision-making. The interactive Tableau dashboard provides actionable insights that can help improve employee retention, optimize workforce planning, and enhance overall organizational performance.

# Tableau public : https://public.tableau.com/app/profile/aravind.nimma/viz/Book2_17836204756660/HRAttritionAnalysisDashboard
